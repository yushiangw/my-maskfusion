...
#define STOPWATCH(name, expression)                                                                    \
  do {                                                                                                 \
    ...
    expression const unsigned long long int endTime = Stopwatch::getInstance().getCurrentSystemTime(); \
...
class Stopwatch {
 public:
  ...
  void setCustomSignature(unsigned long long int newSignature) { signature = newSignature; }

  const std::map<std::string, float>& getTimings() { return timings; }
  ...
  void pulse(std::string name) { timings[name] = 1; }
  ...
  void tick(std::string name, unsigned long long int start) { tickTimings[name] = start; }
 ...
 private:
  ...
  virtual ~Stopwatch() { close(sockfd); }
